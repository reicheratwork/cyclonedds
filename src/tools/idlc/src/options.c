...
static int matches_option(
  int opt, const char *optarg, const idlc_option_t *option)
{
  char chr;
  size_t len;
  ...
  if (!optarg || strncmp(option->suboption, optarg, len) != 0)
    return -1;
  chr = option->type ? '=' : '\0';
  return optarg[len] == chr ? (int)len + (chr == '=') : -1;
}
  ...
  switch (option->type) {
    case IDLC_FLAG:
      *option->store.flag = 1;
      break;
    case IDLC_STRING:
      *option->store.string = optarg;
      break;
    case IDLC_FUNCTION:
      return (option->store.function)(option, optarg);
  }
  return 0;
}
